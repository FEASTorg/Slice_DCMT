...
## Deferred: Replace Temporary Closed-Loop with DCMotorServo Stack

Status: Deferred for later task.

### Why

- Current `gen2_closed` path uses an in-firmware PID implementation (`Encoder` + custom control loop).
- Original Gen2 closed-loop behavior used `DCMotorServo` + `DCMotorTacho`.
- We want eventual parity with the legacy closed-loop architecture and tuning model.

### Future Work Items

1. Add `DCMotorServo` and `DCMotorTacho` dependencies in `firmware/platformio.ini` for `gen2_closed`.
...
1. `gen2_closed` compiles and runs using `DCMotorServo`/`DCMotorTacho` (no custom PID loop active).
